...
def modularity(partition, graph, weight='weight', inverted = False):
...
def __one_level(graph, status, weight_key, resolution, random_state, inverted = False):
    """Compute one level of communities
    """
    modified = True
    nb_pass_done = 0
    cur_mod = __modularity(status, resolution, inverted= inverted)
    new_mod = cur_mod
...
def __modularity(status, resolution, inverted = False):
    """
    ...
    return -result if inverted else result
